...
const FourierSeries = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            console.error(err);
            setError(err.response?.data?.detail || "Analysis failed. Please check your signal expression syntax.");
        }
        setLoading(false);
    };
    ...
    const handleAutoDetectPeriod = async () => {
        ...
        try {
            ...
            if (res.data.period !== null) {
                ...
                setError(res.data.message || "Could not detect period");
            }
        } catch (err) {
            console.error(err);
            setError(err.response?.data?.detail || "Period detection failed. Check server connection or syntax.");
        }
    };
